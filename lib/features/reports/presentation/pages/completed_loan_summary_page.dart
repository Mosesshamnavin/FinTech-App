...
class CompletedLoanSummaryPage extends StatefulWidget {
  ...
  @override State<CompletedLoanSummaryPage> createState() => _CompletedLoanSummaryPageState();
}
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<ReportBloc, ReportState>(
        ...
        builder: (ctx, state) => BlocBuilder<SettingsBloc, SettingsState>(builder: (context, ss) {
          ...
          return Scaffold(
            appBar: AppBar(title: const Text('Completed Loan Summary'), leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new, size: 20), onPressed: () => Navigator.pop(context))),
            body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Center(child: state is ReportLoading ? const CircularProgressIndicator() : ElevatedButton(onPressed: () => ctx.read<ReportBloc>().add(LoadCompletedLoanSummaryRequested(fromDate: _from.text, toDate: _to.text, line: _lineAll ? null : _line)), style: ElevatedButton.styleFrom(backgroundColor: Colors.lightBlue[300], padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12)), child: const Text('SUBMIT', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))),
