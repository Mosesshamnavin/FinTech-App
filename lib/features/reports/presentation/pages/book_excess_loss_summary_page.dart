    ...
    return BlocProvider(
      ...
      child: BlocConsumer<ReportBloc, ReportState>(
        ...
        builder: (ctx, state) => Scaffold(
          appBar: AppBar(title: Text('Book Excess/Loss Summary'.tr()), leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new, size: 20), onPressed: () => Navigator.pop(context))),
          body: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const SizedBox(height: 16),
            _dp('From Date', _from), const SizedBox(height: 16), _dp('To Date', _to),
            const SizedBox(height: 32),
            Center(child: state is ReportLoading ? CircularProgressIndicator() : ElevatedButton(onPressed: () => ctx.read<ReportBloc>().add(LoadBookExcessLossSummaryRequested(fromDate: _from.text, toDate: _to.text)), style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).colorScheme.primary, padding: EdgeInsets.symmetric(horizontal: 24, vertical: 12)), child: Text('SUBMIT', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))),
