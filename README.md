...
- **Auth & Security**:
  ...
  - Strict route guarding to prevent login bypasses.
  - In-app **Change Password** directly linked to Hasura.
  - Unified Settings UI with direct toggles for **Biometric Auth** and **Security Alerts**, now utilizing customized prompt messages.
- **Optimistic State Management**:
  - Blazing fast UI updates powered by a customized BLoC architecture.
  - Creates, updates, and deletes (for Lines, Areas, Expense Types, etc.) instantly reflect on the screen without waiting for expensive backend network refreshes.
  - Graceful UI error handling for Hasura Database Permission drops.
- **Media & Uploads**:
  - Integrated `image_picker` for seamless device camera and gallery selection (e.g. for UPI QR Codes).
...
The application operates on a clean, centralized flow designed for field agents and administrators:

1. **Authentication Flow**
   - Users must authenticate via the secure Login screen. 
   - JWT tokens are retrieved from Hasura and securely cached using `SharedPreferences`.
   - Optional Biometric verification (Fingerprint / FaceID) provides secondary security.

2. **Core Settings & Context (Global BLoC)**
   - Upon logging in, the app triggers a global `LoadSettingsRequested` event.
   - This fetches all crucial reference data (`expense_types`, `investment_types`, `areas`, `line_types`, `lines`) into the app's memory.
   - Any modifications to these settings use **Optimistic Updates**, instantly injecting the change into the UI while syncing with Hasura in the background.

3. **Daily Operations (Vasool Flow)**
   - **Data Entry**: Agents navigate to the target `Line` and create `Collections` or `CashOuts`.
   - **Offline/Error Safety**: If an agent lacks Hasura `insert/select` permissions or loses internet, the UI elegantly catches the GraphQL errors and displays red warning messages instead of breaking silently.
   - **Communications**: Following a successful vasool (collection), the agent is immediately prompted to send an SMS or WhatsApp receipt (parsed from dynamic SharedPreferences templates).

4. **Reporting & Analytics**
   - Data flows directly from Hasura into real-time reporting BLoCs.
   - Managers can export line reports directly to CSV files on the device, or import bulk data via CSV templates.
